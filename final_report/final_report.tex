		...
		transform shape, align=center, state/.style={circle, draw, minimum size=2.3cm}]
			...
			\node[initial,state] (FG) {Finding \\ Gargamel};
	 		\node[state]         (AG) [right of=FG] {Approaching \\ Gargamel};
  			\node[state]         (FW) [below of=FG] {Finding \\ woman};
  			\node[state]         (AW) [right of=FW] {Approaching \\ woman};
  			\node[state]         (FC) [right of=AG] {Finding \\ cylinder};
  			\node[state]         (AC) [right of=FC] {Approaching \\ cylinder};
  			\node[state]         (FR) [below of=AC] {Finding \\ ring};
  			\node[state]         (AR) [left of=FR] {Approaching \\ ring};
  			
  			\path (FG) edge[bend left] node {Gargamel \\ found} (AG);
  			
  			\path (AG) edge[bend right] node {Preferences \\ received} (FW);
  			\path (AG) edge[bend left] node {Woman according to \\ preferences found} (FC);
  			...
  			\path (AR) edge[bend left] node {Ring \\ approached } (AW);
  			
  			\path (AW) edge[bend right, right] node {Woman willing \\ to marry Gargamel} (AG);
  			\path (AW) edge[bend right, above] node {Woman not willing \\ to marry Gargamel} (FW);
  			
  			\path (FW) edge[bend right, below] node {Woman according to \\ preferences found} (AW);
		\end{tikzpicture}
