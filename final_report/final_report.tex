...
\begin{document}
	...
	In homework3 we had to train a model that could correctly classify twenty-one given faces. To solve this problem we decided to use a neural network that transforms images into encodings, putting them into embedding space. We used images of faces taken in the gazebo simulator. Then we trained different classifiers on the obtained encodings. Below are the results of our testing. \\
	...
	As we can see most of the classifiers perform really well. For erazem we decided to use the svm model with a linear kernel and parameter c equal to 0.05. Our classifier performed really well in the gazebo simulator as well. \\
