		...
		transform shape, align=center, state/.style={circle, draw, minimum size=2.3cm}]
			...
			\node[initial,state] (FG) {Finding \\ Gargamel};
	 		\node[state]         (AG) [right of=FG] {Approaching \\ Gargamel};
  			...
  			\node[state]         (AW2) [left of=AR] {Approaching \\ woman 2};
  			...
  			\path (AW2) edge[bend right, right] node {Woman willing \\ to marry Gargamel} (AG);
  			\path (AW2) edge[bend right, above] node {Woman not willing \\ to marry Gargamel} (FW);
