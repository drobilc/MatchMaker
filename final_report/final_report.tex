...
\begin{document}
	...
	% Popravi, tole je blo napisano ko sem bil zelo utrujen
	...
	It should also be noted that similar process is applied to the approaching point. Approaching point is computed for each face detection and is then averaged. In figure \ref{fig:inaccurate_detections}, this point is coloured yellow. \\

	% world position not defined
	
	% not ok, zdj se bere kot da nardimo povprečje (detekcija + app.point)/2, kar seveda ni res.
	% Opomba, po novem se računa točno na tak način kot si napisala zgoraj torej (prejšnja + nova) * 0.5 tko da lahko pustimo
