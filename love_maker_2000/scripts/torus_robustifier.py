...
class TorusRobustifier(object):
    ...
    def __init__(self):
        ...
        self.raw_cylinder_subscriber = rospy.Subscriber('/torus_detections_raw', PoseStamped, self.on_cylinder_detected, 10)

        # After determining cylinder position, send a pose to movement_controller node
        self.cylinder_publisher = rospy.Publisher('/torus_detections', PoseStamped, queue_size=10)

        self.cylinder_detections = []

        self.markers_publisher = rospy.Publisher('torus_markers_raw', MarkerArray, queue_size=1000)
